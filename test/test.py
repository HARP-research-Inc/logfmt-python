...
class TestSimple(TestCase):
    ...
    def test_discard_none(self):
        formatter = LogfmtFormatter(colorize=False, discard_none=True)
        logger, stream = setup_logger(formatter, name="discard_none")
        logger.debug("Hello, world!", extra={"data": None})
        value = stream.getvalue()
        self.assertIn("level=DEBUG", value)
        self.assertNotIn("data=", value)
        self.assertIn("function=test_discard_none", value)
        self.assertIn("name=discard_none", value)

    def test_msg_regex(self):
        formatter = LogfmtFormatter(colorize=False, msg_regex=r"^(?P<a>\d+),(?P<b>\d+),(?P<c>\d+)$")
        logger, stream = setup_logger(formatter, name="msg_regex")
        logger.debug("42,43,44")
        value = stream.getvalue()
        self.assertIn("level=DEBUG", value)
        self.assertIn("function=test_msg_regex", value)
        self.assertIn("name=msg_regex", value)
        self.assertIn("a=42", value)
        self.assertIn("b=43", value)
        self.assertIn("c=44", value)
        self.assertNotIn("message=", value)

    def test_msg_regex_no_match(self):
        formatter = LogfmtFormatter(colorize=False, msg_regex=r"^(?P<a>\d+),(?P<b>\d+),(?P<c>\d+)$")
        logger, stream = setup_logger(formatter, name="msg_regex_no_match")
        logger.debug("42,43")
        value = stream.getvalue()
        self.assertIn("level=DEBUG", value)
        self.assertIn("function=test_msg_regex_no_match", value)
        self.assertIn("name=msg_regex_no_match", value)
        self.assertIn("message=42,43", value)
        self.assertNotIn("a=", value)
        self.assertNotIn("b=", value)
        self.assertNotIn("c=", value)

    def test_auto_space_quotes(self):
        formatter = LogfmtFormatter(colorize=False)
        logger, stream = setup_logger(formatter, name="auto_space_quotes")
        logger.debug("Hello, world!", extra={"data": "Hello,world!"})
        value = stream.getvalue()
        self.assertIn("level=DEBUG", value)
        self.assertIn('message="Hello, world!"', value)
